package com.test.coding.baekjoon.arbitrary_precision;
 ...
 *
 * $_1914 의 설명
 * 패키지: com.test.coding.algorithm.arbitrary_precision
...
public class $_1914 {
    ...
    public static void  main(String[] args){
        try(BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
            ...
            bw.write(String.valueOf(hanoiOfTop(N))); //총 옮긴 횟수
            bw.newLine();
            bw.flush();

            if(N <= 20) hanoiOfTop(N, 1, 2, 3); //옮기는 과정
        }catch(IOException e){
            e.printStackTrace();
        }
    }

    static BigInteger hanoiOfTop(int n){
    ...
    static void hanoiOfTop(int n, int start, int temp, int end) {
        if(n == 1){
        ...
        hanoiOfTop(n - 1, start, end, temp);
        //System.out.printf("%d %d", start, end); //FIXME 해당 코드는 메모리초과의 원인이 됨
        System.out.println(start + " " + end); //A -> C
        hanoiOfTop(n - 1, temp, start, end);
    }
